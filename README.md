...
* **JavaFX window does not open:** run `gradle --no-daemon clean runSimulation`; ensure Maven Central is reachable so Gradle can download JavaFX. The project intentionally contains no JavaFX stubs.
...
## Corrected Workflow Details

* **Ear selection is active backend state**: `Right` runs only RIGHT, `Left` runs only LEFT, and `Both` runs RIGHT first and then LEFT. Changing the ear mode resets the session with the updated ear list.
* **Frequency progression is cursor-based**: the clinical order can include the 1000 Hz retest without creating an infinite loop. Standard required frequencies 250, 500, 1000, 2000, 4000, and 8000 Hz are reached.
* **Pause/Resume**: Pause blocks tone presentation and automatic simulation; Resume returns the session to `READY`. Stop still requires Reset before continuing.
* **Runtime configuration**: biomedical parameters are loaded from `config/audiometry-config.json` at startup. Invalid or missing config safely falls back to Java defaults and logs a GUI warning.
* **JSON export**: session JSON is generated with Jackson and is parseable structured JSON.
