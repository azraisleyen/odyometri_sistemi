...
## Ear Selection and Commands

Serial tone commands use the current backend ear from `TestState`. Selecting Right, Left, or Both in the GUI updates the session configuration, so commands match the selected/current ear, for example `TONE;EAR=LEFT;FREQ=1000;DB=40;DURATION_MS=1000` in left-only mode.
