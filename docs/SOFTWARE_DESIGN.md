...
The application uses real JavaFX dependencies from Gradle. `gradle runSimulation` opens the desktop GUI in simulation mode, while `gradle run` opens the same GUI with the real serial gateway available for COM-port integration.
