...
## Ear Modes, Pause/Resume, and Config

The Ear selector changes backend state: Right runs only RIGHT, Left runs only LEFT, and Both runs RIGHT then LEFT. Pause stops new tone presentation and automatic simulation until **Resume test** is pressed. Stop requires **Reset test**. Biomedical defaults are loaded from `config/audiometry-config.json` at startup.
