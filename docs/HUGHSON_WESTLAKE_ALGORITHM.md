...
## Frequency Progression and Retest

The clinical order may include a duplicate 1000 Hz retest. The implementation advances with a frequency-plan cursor, not by searching for the current frequency value, so the workflow reaches 500 Hz and 250 Hz and can complete. When retest saving is disabled, duplicate audiogram rows for the same ear/frequency are not added.
