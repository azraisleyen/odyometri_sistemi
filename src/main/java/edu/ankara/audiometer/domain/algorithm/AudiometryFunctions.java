...
public final class AudiometryFunctions {
    ...
    /** Pure Hughson-Westlake intensity transition: heard => down, not heard/timeout => up. */
    ...
    /** Legacy value-based helper retained for callers that do not need duplicate-frequency cursor handling. */
    public static Optional<FrequencyHz> nextFrequency(FrequencyHz current, FrequencyPlan plan) {
        ...
        for (int index = 0; index < order.size() - 1; index++) {
            if (order.get(index).equals(current)) {
    ...
    /** Index-based progression supports duplicate values such as the 1000 Hz retest without looping. */
    ...
    public static TonePresentation createTonePresentation(TestState state) {
        boolean ascending = state.direction() == PresentationDirection.ASCENDING || state.phase() == TestPhase.ASCENDING_SEARCH;
    ...
    public static AudiogramPoint createAudiogramPoint(TestState state, IntensityDbHL threshold) {
        long presentationCount = state.presentations().stream()
                .filter(presentation -> presentation.ear() == state.currentEar()
                        && presentation.frequency().equals(state.currentFrequency()))
                .count();
    ...
    public static String classifyHearingLevel(IntensityDbHL threshold) {
        ...
        if (value <= 25) {
            return "Normal/near-normal (educational classification)";
        }
        if (value <= 40) {
            return "Mild";
        }
