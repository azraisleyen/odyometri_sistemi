...
public final class MainController {
    ...
    public MainController(AudiometryUseCase useCase, boolean simulationMode) {
        this(useCase, simulationMode, 9600, List.of());
    }

    public MainController(AudiometryUseCase useCase, boolean simulationMode, int defaultBaudRate, List<String> startupWarnings) {
    ...
    public Parent root() {
        ...
        startupWarnings.forEach(warning -> log.add("Config warning: " + warning));
        return root;
    }
    ...
    private void autoSimulate() {
        ...
        if (command.isOk()) {
            ...
            log.add("Auto simulation skipped because no tone could be presented");
            return;
        }
    ...
    private void earModeChanged(EarTestMode mode) {
        ...
        lastThresholdCount = 0;
        log.add("Ear mode changed to " + mode + "; session reset with updated ear list");
    }
