...
public final class AudiogramChartPane extends TitledPane {
        ...
        x.setTickLabelFormatter(new StringConverter<>() {
            ...
            public String toString(Number value) {
                ...
                return FREQUENCY_POSITIONS.entrySet().stream()
                        .filter(entry -> entry.getValue() == rounded)
    ...
    private void addSeries(String name, Ear ear, Audiogram audiogram) {
        ...
        audiogram.points().stream()
                ...
                .filter(point -> FREQUENCY_POSITIONS.containsKey(point.frequency().value()))
                .sorted(Comparator.comparing(point -> FREQUENCY_POSITIONS.get(point.frequency().value())))
                .forEach(point -> {
                    XYChart.Data<Number, Number> data = new XYChart.Data<>(
                            FREQUENCY_POSITIONS.get(point.frequency().value()),
                            -point.thresholdDbHL().value()
                    );
