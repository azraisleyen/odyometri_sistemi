...
public final class SimulationService {
    ...
    public TestState autoRespond(TestState state) {
        boolean heard = state.currentIntensity().value() >= thresholdFor(state.currentEar(), state.currentFrequency()).value();
        return TestStateReducer.updateState(state, heard ? new ProtocolEvent.Response() : new ProtocolEvent.NoResponse());
    }
